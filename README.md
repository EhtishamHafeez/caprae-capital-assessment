...
An enhancement built on top of the [SaaSquatch Leads](https://www.saasquatchleads.com/) concept for the Caprae Capital Full Stack Developer pre-work challenge — a lead-generation dashboard that layers **three AI-powered features**, a **CRM webhook integration**, and a **transparent ICP-fit scoring engine** on top of the core scrape → enrich → save → export workflow.
...
5. **CRM webhook export.** Paste any inbound webhook URL once (Zapier, Make, n8n, HubSpot, Salesforce Flow, or a plain endpoint) and send a lead's full record there with one click — real server-to-server delivery, not a mocked integration. See **CRM integration** below.
...
- **Revenue/employee bounds are hard filters, not just a scoring hint — a real bug found via live testing.** `minRevenue`/`maxRevenue`/`minEmployees`/`maxEmployees` were originally used only inside `scoreLead()`, as a closeness-to-range signal — a lead well under a stated "$5M+" floor could still appear if it scored well on other dimensions, and did: a live natural-language search for "manufacturing leads in Ohio over $5M revenue" was returning leads at $4.5M and $4.8M, which is actively misleading rather than just imprecise, since the AI's own explanation banner claimed "revenue above $5M." `queryLeads()` in `leads-repo.ts` now excludes out-of-range rows before scoring runs at all; `scoreLead()`'s closeness formula still runs on the filtered set, now rewarding how centered a lead is within the range rather than deciding whether it qualifies. Covered by regression tests in `leads-repo.test.ts` — see **Testing**.
...
Rather than build OAuth against one specific CRM (which would need a real developer account and sandbox I don't have, and would only work for that one platform), leads export via a **generic inbound webhook** — the pattern every CRM and no-code tool (Zapier, Make, n8n, HubSpot workflows, Salesforce Flow, a plain endpoint) already supports. Paste the URL once in a lead's drawer; every "Send this lead to CRM" after that is a real POST, not a simulated one.

- **Where the URL lives**: `localStorage`, not the server. This is a single-tenant demo app with no user accounts, so there's no meaningful server-side place to keep a per-viewer setting — see `lib/use-crm-webhook.ts`.
- **What gets sent**: the same fields as the CSV export (company info, contact data, growth signals, description) plus the ICP score and tier, as JSON — see `POST /api/leads/[id]/crm-send`.
- **Why it's a server-side proxy, not a direct browser POST**: two reasons. Many webhook receivers don't set CORS headers for browser-originated requests, so a direct client POST would silently fail against some targets; and routing it through our own API route means the request can be rate-limited and validated the same way the AI routes are.
- **SSRF guard**: the webhook URL is user-supplied and our server makes the request on the caller's behalf, so it's validated before use — `https://` only, and common private/internal hostname patterns (`localhost`, `127.0.0.1`, the RFC1918 ranges, link-local, `.local`/`.internal`) are rejected (`lib/webhook-validation.ts`, tested). This doesn't resolve DNS, so a public hostname that only *resolves* to a private IP (DNS rebinding) would slip through — a production version would check the resolved IP, not just the hostname string; noted here rather than silently left as a gap.
...
80 tests (Vitest) covering the logic that's actually risky to get wrong:
...
- **`webhook-validation.test.ts`** — the CRM webhook's SSRF guard: accepts real public webhook hosts, rejects non-HTTPS, localhost, loopback, private IPv4 ranges, link-local addresses, `.local`/`.internal` hostnames, and malformed input, without a false positive on a public IP that merely starts similarly to a private range (`172.32.x.x` vs. the blocked `172.16–31.x.x`).
- **`db.test.ts`** — the seed-time dedup (`dedupeByDomain`): collapses same-domain rows, case-insensitive and whitespace-trimmed, keeps the higher-completeness row regardless of input order, keeps the first-seen row on a tie, and leaves rows with no website or with distinct domains untouched.
...
- **`leads-repo.test.ts`** — integration tests against the real seeded SQLite database: filtering, full-text search, pagination boundaries, sorting, the minRevenue/maxRevenue/minEmployees/maxEmployees hard-filter regression (see **Design decisions**), and the save/unsave round trip (with cleanup so tests don't leave state behind).
...
src/
  app/
    ...
    api/
      ...
      leads/[id]/crm-send/route.ts # POST — send a lead to a user-configured CRM webhook
  ...
  lib/
    db.ts                  # SQLite connection + schema + CSV seed + dedup
    ...
    webhook-validation.ts     # SSRF guard for the CRM webhook feature
    use-crm-webhook.ts         # Client hook: persisted per-viewer webhook URL
    cache.ts                 # In-memory query cache + per-feature AI response caches
    rate-limit.ts             # Fixed-window rate limiter for the AI/webhook routes
